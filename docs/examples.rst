...
The same code with a lock eliminates the race. Marker schedules are the
wrong tool to prove it: the marker executor serializes execution between
markers, so a thread that blocks on the lock while its peer is paused at a
marker stalls the schedule (the run fails with a ``TimeoutError`` rather
than certifying anything). Use DPOR instead — it understands lock blocking
and explores every meaningfully different interleaving:
...
.. code-block:: python
   ...
   result = frontrun.explore(
       setup=lambda: SyncedBankAccount(balance=100),
       workers=[lambda a: a.transfer(50), lambda a: a.transfer(50)],
       invariant=lambda a: a.balance == 200,
   )
   result.assert_holds()  # passes: the lock serializes the critical section
...
.. code-block:: python
   ...
   executor = TraceExecutor(schedule)
   executor.run({
       "task1": functools.partial(account.transfer, 50),
       "task2": functools.partial(account.transfer, 50),
   })

   assert account.balance == 150  # one transfer lost

Pass coroutine-returning callables (``functools.partial`` over the async
method, or the bound method itself for zero-argument calls) so the executor
recognizes them as async tasks.  A plain ``lambda`` wrapping the coroutine
call is classified as a *sync* worker; the returned coroutine would never be
awaited, and ``TraceExecutor`` fails closed with a ``TypeError`` rather than
reporting success without executing anything.
