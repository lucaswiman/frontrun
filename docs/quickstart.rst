...
Finding a race automatically
----------------------------

The front door is :func:`frontrun.explore`: hand it a way to build shared
state, some workers, and an invariant, and DPOR systematically explores every
meaningfully different thread interleaving until one breaks the invariant or
all of them are proven safe. No annotations needed:
...
Run it through the ``frontrun`` CLI wrapper --- plain ``pytest`` skips
...
The failure output names the conflicting accesses, shows the interleaved
source-line trace, and replays the counterexample schedule to confirm it
reproduces deterministically. See :doc:`dpor_guide` for the practical guide,
and :doc:`approaches` for the other exploration strategies (random bytecode
exploration, marker schedule exploration).

The rest of this guide covers **trace markers** --- the fully manual approach,
