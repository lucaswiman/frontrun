...
Imported by a spawned ``worker_main`` child after it connects and installs
interception. Writes far more than a pipe buffer (64 KiB on Linux) to stderr to
prove the launcher's capture cannot deadlock the child, then exits nonzero so
``diagnose()`` reads the capture.
