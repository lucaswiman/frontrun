...
def _lock_event_schedule_index(scheduler: DporScheduler, thread_id: int, trace_length: int) -> int:
    """Return the reporting actor's schedule slot without rescanning the trace."""
    path_id = scheduler._last_scheduled_path_id_by_thread.get(thread_id)
            ...
            try:
                if event == "lock_wait":
                    with engine_lock:
                        ...
                        _wait_idx = (
                            _lock_event_schedule_index(scheduler, thread_id, _trace_len_wait)
                ...
                if event == "lock_acquire":
                    with engine_lock:
                        ...
                        _acq_idx = (
                            _lock_event_schedule_index(scheduler, thread_id, _trace_len_acq)
                            if scheduler._lock_event_collector is not None
                            else None
                        )
                ...
                if event == "lock_release":
                    with engine_lock:
                        ...
                        _rel_idx = (
                            _lock_event_schedule_index(scheduler, thread_id, _trace_len_rel)
                            if scheduler._lock_event_collector is not None
                            else None
                        )
